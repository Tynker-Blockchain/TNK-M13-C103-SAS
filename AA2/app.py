...
def checkPaymentStatus():
    ...
    # Check the if ID is present in PaymentID_Status and it's status is success or not.
    if "ID" in PaymentID_Status and PaymentID_Status["Status"] == "success":
        #Update the response as success with payment ID
        response = {
            "status": "success",
            "paymentID": PaymentID_Status["ID"]
        }
    else:
        #Update the responce as proccessing
        response = {
            "status": "processing"
        }

    # Pass the response instead of True
